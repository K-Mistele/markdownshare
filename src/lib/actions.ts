...
export async function createDocument(document: DocumentInsert): Promise<Document | null> {
    ...
    try {
        const result = await db.insert(documents).values(documentWithAuthor).returning()
    ...
    try {
        ...
        if (!isAuthor) {
            ...
            const collaboration = await db
                ...
                .where(and(eq(documentCollaborators.documentId, documentId), eq(documentCollaborators.userId, user.id)))
                .limit(1)

            if (!collaboration[0] || collaboration[0].permission !== 'admin') {
                throw new Error('Access denied: only document author or admin collaborators can add collaborators')
            }
        }
...
export async function updateCollaboratorPermission(
    ...
    try {
        ...
        if (!isAuthor) {
            ...
            if (!collaboration[0] || collaboration[0].permission !== 'admin') {
                throw new Error('Access denied: only document author or admin collaborators can update permissions')
            }
        }
...
export async function createComment(comment: CommentInsert): Promise<Comment | null> {
    ...
    const { canAccess } = await canUserAccessDocument(comment.documentId, user.id)
    if (!canAccess) {
    ...
    try {
        const result = await db.insert(comments).values(commentWithUser).returning()
        return result[0] || null
    } catch (error) {
    ...
    try {
        ...
        if (!isCommentOwner) {
            ...
            if (!document[0] || document[0].authorId !== user.id) {
                throw new Error('Access denied: can only delete your own comments or comments on your documents')
            }
        }
